...
const SUGGESTIONS = [
  ...
  "What is the average price over 6 months?",
]

export default function ChatPanel({ symbol, range }: Props) {
  const [input, setInput]   = useState('')
  const { messages, streaming, sendMessage, clearMessages } = useChat(symbol, range)
      ...
      <div className="px-4 py-3 border-t border-gray-100">
        <div className="flex gap-2">
          <input
            ...
            placeholder={streaming ? 'Generating response...' : `Ask about ${symbol}...`}
            disabled={streaming}
            className="flex-1 text-sm border border-gray-200 rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-gray-50 disabled:text-gray-400"
          />
          <button
            onClick={handleSend}
        ...
        <p className="text-xs text-gray-400 mt-1.5">
          All responses are verified against real price data.
        </p>
      </div>
    </div>
  )
