...
def make_messages(n: int) -> list[dict]:
    ...
    for i in range(n):
        ...
        messages.append(
            {"role": role, "content": f"Message {i + 1} about AAPL stock price history"}
        )
    return messages
...
async def test_long_history_compressed_to_summary():
    ...
    with patch(
        "investorai_mcp.llm.history.call_llm",
        new=AsyncMock(return_value="User discussed AAPL prices."),
    ):
        result = await compress_history(messages)

    # should be summary + last 5 messages
    assert (
        len(result) == EXPLICIT_WINDOW + 1
    )  # Result should contain summary + last EXPLICIT_WINDOW messages
...
async def test_summary_is_first_message():
    ...
    with patch("investorai_mcp.llm.history.call_llm", new=AsyncMock(return_value=summary_text)):
        result = await compress_history(messages)
...
async def test_summary_contains_earlier_conversation_label():
    ...
    with patch(
        "investorai_mcp.llm.history.call_llm", new=AsyncMock(return_value="User discussed stocks.")
    ):
        result = await compress_history(messages)

    assert (
        "[Earlier conversaiton summary]" in result[0]["content"]
    )  # Summary message should contain label indicating it's a summary of earlier conversation
...
async def test_compression_reduces_message_count():
    ...
    assert len(result) < len(
        messages
    )  # Compressed history should have fewer messages than original
...
def test_token_count_multiple_messages():
    ...
    messages = [{"role": "user", "content": "a" * 200}, {"role": "assistant", "content": "b" * 200}]
