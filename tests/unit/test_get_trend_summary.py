...
def patch_pipeline(
    ...
    return (
        patch(
            "investorai_mcp.tools.get_trend_summary.get_price_history",
            new=AsyncMock(return_value=price_result),
        ),
        patch(
            "investorai_mcp.tools.get_trend_summary.get_news",
            new=AsyncMock(return_value=_MOCK_NEWS),
        ),
        patch(
            "investorai_mcp.tools.get_trend_summary.get_stock_info",
            new=AsyncMock(return_value=_MOCK_STOCK_INFO),
        ),
        patch(
            "investorai_mcp.tools.get_trend_summary.get_sentiment",
            new=AsyncMock(return_value=_MOCK_SENTIMENT),
        ),
        patch(
            "investorai_mcp.tools.get_trend_summary.call_llm",
...
def test_detect_three_companies():
    result = _detect_all_symbols_from_question("Compare Tesla, Apple and Microsoft")
...
async def test_meta_question_no_llm_call():
    ...
    with patch("investorai_mcp.tools.get_trend_summary.call_llm") as mock_llm:
        result = await get_trend_summary("AAPL", question="what stocks do you support?")
...
async def test_specific_date_fast_path():
    ...
    with (
        ...
        result = await get_trend_summary("AAPL", question="What was the price on 2026-03-28?")
...
async def test_symbol_detected_from_question():
    ...
    with p1, p2, p3, p4, p5:
        result = await get_trend_summary("AAPL", question="How is TSLA doing?")
...
async def test_6_month_question_uses_6m_range():
    ...
    with p1, p2, p3, p4, p5:
        result = await get_trend_summary(
            "AAPL", question="What is the average price over 6 months?"
        )
...
async def test_date_range_empty_rows_returns_no_data_summary():
    ...
    with (
        ...
        patch(
            "investorai_mcp.tools.get_trend_summary.call_llm", new=AsyncMock(return_value="summary")
        ),
    ):
        # "from May 2023 to May 2025" resolves to a range; rows all pre-date it
        result = await get_trend_summary(
            "AAPL", question="How did AAPL do from May 2025 to Jun 2025?"
        )
    assert result.get("validation_passed") is True
...
async def test_specific_date_before_history():
    ...
    with (
        ...
        result = await get_trend_summary("AAPL", question="What was the price on 2021-04-22?")
    assert result.get("validation_passed") is True
...
async def test_date_range_question_fetches_data_for_range():
    ...
    with (
        ...
        result = await get_trend_summary(
            "AAPL", question="How did AAPL do from May 2023 to May 2025?"
        )
    assert "summary" in result
...
async def test_history_is_compressed_when_provided():
    ...
    with (
        ...
        patch(
            "investorai_mcp.tools.get_trend_summary.compress_history",
...
async def test_multi_stock_resolved_date_returns_prices():
    ...
    with (
        ...
        result = await get_trend_summary(
            "AAPL", question="What were the prices of AAPL and MSFT on 2026-03-27?"
        )
    mock_llm.assert_not_called()
    assert result.get("multi") is True
...
async def test_multi_stock_news_focus_returns_sentiments():
    ...
    with (
        ...
        result = await get_trend_summary("AAPL", question="Latest news for AAPL and MSFT stock")
    assert result.get("multi") is True
...
async def test_multi_stock_date_range_builds_combined_prompt():
    ...
    with (
        ...
        result = await get_trend_summary(
            "AAPL", question="How did AAPL and MSFT do from May 2023 to May 2025?"
        )
    assert result.get("multi") is True
    assert "summary" in result
...
async def test_ticker_symbol_param_unsupported_no_question_symbols():
    ...
    with patch(
        ...
        result = await get_trend_summary(
            "FAKECORP", question="How has this stock performed recently"
        )
    mock_llm.assert_not_called()
    assert result.get("error") is True
    assert result["code"] == "TICKER_NOT_SUPPORTED"
