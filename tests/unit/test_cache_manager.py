...
async def test_get_or_create_meta_creates_on_first_call(
    cache_manager, seeded_ticker, session
):
...
async def test_get_or_create_meta_returns_existing(
    cache_manager, seeded_ticker, session
):
    # Create initial meta
...
def test_period_to_cutoff_5y():
    ...
    assert abs((cutoff - expected).days) <= 1  # Allow 1 day margin due to leap years and month length variations
...
async def test_get_prices_stale_returns_cached_result(
    cache_manager, seeded_ticker, mock_adapter
):
    result = await cache_manager.get_prices("AAPL", "1Y")
    assert isinstance(result, CacheResult)
...
### upsert prices --------------------------------- -
async def test_upsert_prices_writes_to_db(
    cache_manager, seeded_ticker, session
):
    records = [
...
async def test_upsert_prices_updates_on_conflict(
    cache_manager, seeded_ticker, session
):
...
async def ensure_ticker_exists_returns_existing(
    cache_manager, seeded_ticker
):
...
async def test_ensure_ticker_exists_creates_from_stocks_list(
    cache_manager, session
):
...
async def test_ensure_ticker_exists_unknown_returns_none(
    cache_manager, session
):
...
async def test_update_meta_success_clears_stale(
    cache_manager, seeded_ticker, session
):
    meta = await cache_manager._get_or_create_meta("AAPL", "price_history")
    assert meta.is_stale is True
...
async def test_update_meta_error_increments_error_count(
    cache_manager, seeded_ticker, session
):
    meta = await cache_manager._get_or_create_meta("AAPL", "price_history")
