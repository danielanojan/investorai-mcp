...
def test_extracts_price_with_comma():
    assert 1234.56 in extract_numbers("priced at $1,234.56")


def test_extracts_percentage():
    numbers = extract_numbers("up 12.3% this year")
    assert 12.3 in numbers


def test_extracts_negative_percentage():
    numbers = extract_numbers("down 5.4% this month")
    assert 5.4 in numbers
...
def test_no_numbers_returns_empty():
    assert extract_numbers("AAPL is a technology company") == []


def test_does_not_extract_large_numbers():
    numbers = extract_numbers("market cap of 3 trillion dollars")
    assert not any(n > 100_000 for n in numbers)


def test_deduplicates_numbers():
...
def test_valid_response_passes(sample_stats):
    response = (
        "AAPL started at $150.00 [source: DB • 2025-04-01] "
...
def test_no_numbers_passes(sample_stats):
    response = "AAPL is a technology company listed on NASDAQ."
    result = validate_response(response, sample_stats)
    assert result.passed is True


def test_idk_response_passes_through(sample_stats):
    """Honest IDK responses must pass through unchanged."""
    response = "I don't have reliable data to answer this question."
    result = validate_response(response, sample_stats)
    assert result.passed is True
    assert result.response == response


def test_cannot_answer_passes_through(sample_stats):
    response = "I cannot answer this from the data provided."
    result = validate_response(response, sample_stats)
    assert result.passed is True


def test_not_available_passes_through(sample_stats):
    response = "That information is not available in the provided data."
    result = validate_response(response, sample_stats)
    assert result.passed is True
...
def test_tolerance_boundary_passes(sample_stats):
    # Within 2% tolerance
    close_value = round(sample_stats.end_price * 1.015, 2)
...
def test_just_outside_tolerance_fails(sample_stats):
    # 10% above end_price — far enough from high_price (182.50) to be a violation
    far_value = round(sample_stats.end_price * 1.10, 2)
    response = f"AAPL closed at ${far_value}."
    result = validate_response(response, sample_stats)
    assert result.passed is False
...
def test_tolerance_is_2_percent():
